...
const openEditModal = (product) => {
  ...
  const oldQty = storeProducts.find(p => p.upc === upc)?.products_number ?? 0;
      ...
      .then((created) => {
        setStoreProducts(prev =>
          prev
            .map(p =>
              p.upc === upc
                ? { ...p, products_number: oldQty - newQty }
                : p
            )
            .concat({
              upc: created.upc,
  ...
  fetch(
    `http://localhost:5000/api/store_products/?upc=${encodeURIComponent(upc)}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
    ...
    const hasPromoSibling = selectedStoreProduct &&
  storeProducts.some(p =>
    p.id_product === selectedStoreProduct.id_product &&
    p.promotional_product
  );
const showPromoCheckbox = selectedStoreProduct &&
