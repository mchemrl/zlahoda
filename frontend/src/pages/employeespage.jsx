    ...
    const handleInputChange = () => {
        ...
        if (employeeRole === "All") {
            const matches = allEmployees
                .filter(employee =>
                    (employee.empl_surname || "").toLowerCase().includes(searchTerm)
                )
                .map(employee => employee.empl_surname);
        ...
        const matches = allEmployees
            .filter(employee => {
                const roleMatch = employee.empl_role === employeeRole;
                const surnameMatch = (employee.empl_surname || "").toLowerCase().includes(searchTerm);
                return roleMatch && surnameMatch;
            })
            .map(employee => employee.empl_surname);
        ...
        fetch(url, {credentials: "include"})
            ...
            .then(data => {
                ...
                if (allEmployees.length === 0) {
                    setAllEmployees(Array.isArray(data) ? data : [data]);
                }
            })
    ...
    const handleAddEmployee = () => {
        ...
        fetch("http://localhost:5000/api/employees/", {
            ...
            .catch(err => setErrorModal({
                ...
                message: "Make sure you entered correct data and required fields are not empty."
            }));
    };
        ...
        <div
            ...
            {addEmployeeModalOpen && localStorage.getItem("role") === "Manager" && (
                <div className="fixed inset-0 flex items-center justify-center backdrop-blur-sm z-50">
                    <div
                        className="bg-[#FFF3EA] rounded-2xl shadow-lg p-6 w-11/12 max-w-4xl relative max-h-[90vh] overflow-y-auto">
                        <button
            ...
            <main className="flex-grow flex flex-col w-full h-screen overflow-hidden px-8 py-8">
                <div className="w-full flex space-x-6 mb-4">
                    <div className="flex-2 relative w-full">
                        ...
                        />
                        {suggestions.length > 0 && inputFocused && searchValue.length >= 0 && (
